...
<%!	// 선언부 (declaration, 멤버 필드 및 멤버 메서드를 선언/정의 한다)
	// 선언부를 쓰지 않고 private을 없애면, 멤버필드가 아니고 지역변수가 되기 때문에 값이 유지되지 않고 날라감
	
	private ArrayList<FoodDTO> list = new ArrayList<>();
%>

<%	// 스클립틀릿 내부 코드는 service() 함수 내부 내용이다
	// 따라서, 지역변수는 함수가 호출될 때 마다 새로 만들어진다 -> 값이 유지되지 않는다 => 지역변수와 멤버필드 차이점
