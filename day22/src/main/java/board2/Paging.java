...
public class Paging {
	...
	private Paging(int page, int boardCount) {	// boardCount에서 page내용만 불러오기
		...
		this.boardCount = boardCount;	// DAO를 이용하여 파악한 총 게시글의 개수 : 503
		
		perPage = 10;					// 여기 값을 바꾸면 화면에 출력되는 게시물의 개수가 바뀐다!
		offset = (page - 1) * perPage;	// (3 - 1) * 10, 20개의 게시물은 건너뛰고
		fetch = perPage;				// 그 다음 10개를 화면에 출력한다
		
		// 하단에 페이지 번호들을 링크로 출력할 때 필요한 요소들
		pageCount = boardCount / perPage;	// 페이지가 총 몇 개인지 파악해야 한다. 게시물수 / 페이지당 개수 (503 / 10)
		pageCount += (boardCount % perPage != 0) ? 1 : 0;	//  나머지가 있으면 1페이지 추가
		section = (page - 1) / 10;			// (3 - 1) / 10, 0번째 구역(01, 02, 03...)
		...
		if(end > pageCount) {				// 마지막 페이지번호가 10단위로 계산된 값보다 적으면
